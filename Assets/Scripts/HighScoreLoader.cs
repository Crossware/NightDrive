...
public class HighScoreLoader : MonoBehaviour{
    public TextMeshProUGUI allHighScores;
    public TextMeshProUGUI loseScoreInfo;
    ...
    void Start(){
        ...
        loadBasicScore();
        loadHighScores();
        if (isHighScore() && SceneManager.GetActiveScene().name == "Menu Lose")
        {
            newHighScoreInfo.text = "New High Score: " + currentScore;
    ...
    private void loadBasicScore(){
        ...
        if (loseScoreInfo){
            loseScoreInfo.text = "You've travelled a decent bit: " + distanceTravelled + "!";
        }
    ...
    private void loadHighScores(){
        ...
        if (allHighScores){
            foreach (int score in scoreByName.Keys){
                allHighScores.text += createLine(score, scoreByName[score]) + "\n";
                //int score;
                //Debug.Log("Name: " + name);
