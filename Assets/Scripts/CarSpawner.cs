...
public class CarSpawner : MonoBehaviour{
    ...
    private float rightLaneSpeed = 2.0f;
    ...
    void Start(){
        ...
        Car car2 = new ModerateCar(middleCar, adjustedBusPos, despawnPosition, middleLaneSpeed);
        car2.spawn();
        cars.Add(car2);
        middleLane.Add(car2);

        Car car3 = new FastCar(rightCar, rightLanePosition, despawnPosition, rightLaneSpeed);
        car3.spawn();
        cars.Add(car3);
        rightLane.Add(car3);
    }
        ...
        if(cars.Count < maxCarsPerLane * numberOfLanes){
            ...
            if (leftLane.Count < maxCarsPerLane){
                if (leftLaneGap > minLeftLaneGap + gapOffset){
                    Car car = new SlowCar(leftLanePool[leftLane.Count], leftLanePosition, despawnPosition, leftLaneSpeed);
                    car.spawn();
                    cars.Add(car);
                    leftLane.Add(car);
                    leftLaneGap = 0;
            ...
            if (middleLane.Count < maxCarsPerLane){
                if (middleLaneGap > minMiddleLaneGap + gapOffset){
                    Car car = new ModerateCar(middleLanePool[middleLane.Count], middleLanePosition, despawnPosition, middleLaneSpeed);
                    car.spawn();
                    cars.Add(car);
                    middleLane.Add(car);
                    middleLaneGap = 0;
            ...
            if (rightLane.Count < maxCarsPerLane){
                if (rightLaneGap > minRightLaneGap + gapOffset){
                    Car car = new FastCar(rightLanePool[rightLane.Count], rightLanePosition, despawnPosition, rightLaneSpeed);
                    car.spawn();
                    cars.Add(car);
                    rightLane.Add(car);
                    rightLaneGap = 0;
